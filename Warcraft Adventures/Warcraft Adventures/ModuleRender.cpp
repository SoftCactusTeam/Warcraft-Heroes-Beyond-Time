...
bool Render::Awake()
{
	...
	renderer = SDL_CreateRenderer(core->window->window, -1, flags);
	...
	{
		camera.w = core->window->screen_surface->w / core->window->GetScale();
		camera.h = core->window->screen_surface->h / core->window->GetScale();
		camera.x = 0;
		camera.y = 0;
	}

	uint width = 0, height = 0;
	core->window->GetWindowSize(width, height);
...
{
	bool ret = true;
	uint scale = core->window->GetScale();
